...
export default function FlowWave() {
  ...
  useEffect(() => {
    ...
    // Provide a 1×1 black fallback for the unused haloTexture uniform
    const blackPixel = new Uint8Array([0, 0, 0, 255]);
    const haloFallback = new THREE.DataTexture(blackPixel, 1, 1, THREE.RGBAFormat);
    haloFallback.needsUpdate = true;
    finalPass.uniforms.haloTexture.value = haloFallback;
    ...
    const motePts = new THREE.Points(moteGeo, atmoMat);
    motePts.frustumCulled = false;
    motePts.layers.enable(LAYERS.ENTIRE_SCENE);
    scene.add(motePts);
    ...
    function animate() {
      ...
      pointsMat.uniforms.uWaveHeight.value =
        waveHeight * (1 + scrollCurrent * scrollRise);
      ...
      pointsMat.uniforms.uAppear.value = Math.max(
        0,
      ...
      motePts.position.copy(camera.position);
